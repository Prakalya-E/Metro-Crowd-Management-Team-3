...
import { useLiveSocket, type CrowdUpdatePayload } from "@/hooks/useLiveSocket";
...
export default function KPISection() {
  ...
  // Bugfix: this used to have no safety-net poll at all - it relied
  // entirely on a debounced REST refetch fired from the crowd_update
  // socket event below. That meant every "Live Passengers" tick paid a
  // 600ms debounce PLUS a full network round trip, while every other
  // live widget on this page (LiveStationsPanel, CrowdHeatMap, etc, all
  // via useApiData) patches its numbers straight from the socket
  // payload with zero network delay. The two "Live Passengers" figures
  // (this KPI card's total vs. the per-station cards below it) visibly
  // drifted out of sync, and the KPI card felt sluggish next to
  // everything else. Same 30s safety-net pattern the trains query
  // already uses, so a dead/quiet simulator doesn't leave this card
  // frozen if a socket push never arrives (see LiveStationsPanel.tsx
  ...
  // Instant, no-network patch of the RTK Query cache straight from the
  // socket payload - the same source of truth LiveStationsPanel already
  // reads for its per-station "Passengers now" numbers. This is what
  // actually keeps the KPI total moving in lockstep with the rest of
  // the dashboard instead of trailing behind it.
  const applyCrowdUpdate = useCallback(
        ...
        apiSlice.util.updateQueryData(
          ...
          (draft) => {
            for (const u of payload.updates) {
              const row = draft.find((r) => r.station_id === u.station_id);
              if (!row) continue;
              row.current_count = u.current_count;
              row.crowd_level = u.crowd_level as CrowdLevel;
              row.last_updated = u.source_timestamp ?? payload.timestamp;
              row.occupancy_ratio =
                row.capacity > 0 ? row.current_count / row.capacity : row.occupancy_ratio;
            }
          },
        ),
      );
  ...
  const debouncedDelayedRefetch = useDebouncedRefresh(delayed.refetch);
  useLiveSocket({
    crowd_update: applyCrowdUpdate,
    delay_alert: debouncedDelayedRefetch,
  });
  ...
  const openModelPerformance = useCallback(() => router.push("/analytics"), [router]);
