...
export default function DelayFeatureImportanceCard() {
  ...
  return (
    <section className="rounded-3xl border border-border bg-card p-6">
      <div className="mb-4 flex items-center gap-3">
        ...
        <div>
          <h2 className="text-base font-bold">Delay feature importance</h2>
          <p className="mt-0.5 text-xs text-muted">
            Which time/station/crowd signals drive the delay model most
          </p>
        </div>
      </div>
          ...
          {metrics.feature_importance[0] && (
            <div className="mt-4 flex items-start gap-2.5 rounded-xl border border-blue-500/20 bg-blue-500/10 p-3 text-xs text-blue-500">
              ...
              <p>
                ...
                {metrics?.model_name ?? "Random Forest"} delay model. Random
                Forest and XGBoost are shown side by side so you can compare
                how each weighs the same signals.
              </p>
            </div>
          )}
        </>
